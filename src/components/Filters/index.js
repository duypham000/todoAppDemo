...
export default function Filters() {
  ...
  const priorityChangeHandle = (priorities) => {
    setPriority(priorities);
    dispatch(filterSlice.actions.priorityFilterChange(priorities));
  };
  ...
  return (
    <Row justify="center">
      <Col span={24}>
        ...
        <Search placeholder="input search text" value={searchText} onChange={searchChangeHandle} />
      </Col>
      <Col sm={24}>
        <Typography.Paragraph style={{ fontWeight: 'bold', marginBottom: 3, marginTop: 10 }}>
          Filter By Status
        </Typography.Paragraph>
        <Radio.Group value={status} onChange={statusChangeHandle}>
